...
---Shared Attachment = one-side only. Child is unaware of this attachment. Child.Parent=some another Parent, but not us.
---So, non-Individual attachment is just an attachment. Individual one has two subtypes: Strong and Shared.
...
---@field public CopyOnWriteAwaiting EObjectAttachmentFlags [8] Allows us to use the owned object as non-const (Including Shared Attachment). HUGE NOTE: The object pointer inside the slot might possibly be Static even if this flag is set to true. Real Individual Copies are (TreatAsNonConst + !CopyOnWriteAwaiting); False = always static reference
---@field public SharedAttachment EObjectAttachmentFlags [16] Whether to create an individual copy on the first Mutable request (should be set to 0 when it's done). Works for Individual Copies only; but should be set from templates.
---@field public ForceTransient EObjectAttachmentFlags [32] Whether this attachment is Shared: the referenced object is found via World InstanceId (works for WorldInstances with IndividualCopy only)
---@field public FullCOW EObjectAttachmentFlags [12] Slot content is excluded from save regardless of the attached object's UserFlags
---@field public FullShared EObjectAttachmentFlags [20]
