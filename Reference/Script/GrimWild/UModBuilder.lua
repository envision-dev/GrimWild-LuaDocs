---Builds a mod project (editable source folder) into an installed copy under Mods/ or ModPacks/, per the
---project manifest's own 'type' field.
---Created via NewObject, configured via its UPROPERTY fields, then TryBuild() is called once.
---@class UModBuilder : UObject
---@field public ProjectDiskPath string @Required: absolute path to a project folder (under ModProjects/ or ModPackProjects/).
